...
class Trainer(torch.nn.Module):
    ...
    def load_train_objects(self,args):
        # ---- MODELS: ----
        # # load reverb encoder
        # self.model_reverbenc=models.ReverbEncoder(args).to(args.device)
        # # load waveunet 
        # if bool(args.is_vae)==False:
        #     self.model_waveunet=models.waveunet(args)
        # else:
        #     self.model_waveunet=models.varwaveunet(args)
        # # load combined model 
        # self.model_combined=models.CombinedModel(self.model_waveunet,self.model_reverbenc).to(args.device)
        ...
        # ---- LOSS CRITERION: ----
        self.criterion=loss.LossOfChoice(args).to(args.device)
        # load stft loss as a validation loss for all conditions
        args_tmp=args
        args_tmp.losstype="stft"
        self.criterion_val_stft=loss.LossOfChoice(args_tmp).to(args.device)
        args_tmp.losstype="logmel"
        self.criterion_val_logmel=loss.LossOfChoice(args_tmp).to(args.device)
        args_tmp.losstype="early"
        self.criterion_val_early=loss.LossOfChoice(args_tmp).to(args.device)
        args_tmp.losstype="late"
        self.criterion_val_late=loss.LossOfChoice(args_tmp).to(args.device)
